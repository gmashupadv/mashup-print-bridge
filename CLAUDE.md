...
- `zpl-network` — raw TCP port 9100 ZPL for Zebra-emulation label printers (Printex G300, Godex, TSC, Zebra); native barcodes (`^BE` EAN-13, `^BC` Code128) generated by `printing/zpl.ts`, no raster (`label`). `buildLabelZpl` legge lo **stesso modello a elementi** dell'anteprima HTML: mm → dot con `^FO x,y`, testo con `^A0<orient>` + `^FB` (a capo e allineamento), rotazione `N/R/I/B`, linee `^GB`, barcode con `^BY` ricavato dalla larghezza del riquadro
...
- Tests use vitest; coverage spans `config`, `server`, the drivers (`epson-fpmate`, `ditron-streamwec`, `ditron-keycode`, `escpos-network`, `zpl-network`, `os-printer`) and the `printing/` helpers (`barcode`, `label-template`, `label-renderer`, `zpl`, `escpos-encoder`, `paper-info`).
